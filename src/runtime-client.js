...
export const CLIENT_PROTOCOL_MISMATCH_ERROR =
  "The pi-sand runtime protocol is incompatible with this client.";
export const RUNTIME_UNAVAILABLE_ERROR =
...
// before that contract can complete.
...
function validateRuntimeStatus(status) {
  if (status?.protocolVersion !== PROTOCOL_VERSION)
    throw protocolMismatch(
      `daemon reported version ${String(status?.protocolVersion)}.`,
    );
  if (!Number.isInteger(status?.daemonPid) || status.daemonPid <= 0)
    throw new Error("The runtime returned an invalid daemon pid.");
  return status;
}
...
export class RuntimeClient {
  constructor(options = {}) {
    ...
    this.socketPath =
      socketPath ?? env.PI_SAND_SOCKET ?? runtimeSocketPath({ env });
  ...
  async listTasks() {
    ...
    if (!result || !Array.isArray(result.tasks))
      throw new Error("The runtime returned an invalid task list.");
    return result.tasks;
  }
  ...
  async request(method, params = {}, { version = PROTOCOL_VERSION } = {}) {
    if (process.platform !== "linux")
      throw new Error("The pi-sand runtime is supported only on Linux.");
    if (!isAbsolute(this.socketPath))
      throw new Error("The pi-sand runtime socket path must be absolute.");
    ...
    try {
      ...
      if (
        firstError.code === "ambiguous_mutation" ||
        (MUTATING_METHODS.has(method) && firstError.sent)
      )
        throw firstError;
  ...
  async requestSocket(
    ...
    return new Promise((resolveResponse, rejectResponse) => {
      ...
      const timer = setTimeout(
        () => finish(new Error("runtime request timed out")),
        timeoutMs,
      );
      const finish = (error, response) => {
        ...
        if (error && sent && MUTATING_METHODS.has(method)) {
          const unknown = new Error(MUTATION_OUTCOME_UNKNOWN_ERROR, {
            cause: error,
          });
          unknown.code = "ambiguous_mutation";
          unknown.sent = true;
      ...
      socket.on("data", (chunk) => {
        ...
        try {
          ...
          if (response.id !== requestId)
            throw new Error("runtime response id did not match the request");
          finish(null, response);
        } catch (error) {
          finish(
            error.message === "runtime response id did not match the request"
              ? error
              : new Error("runtime returned invalid JSON"),
          );
        }
      });
      ...
      socket.once("close", () => {
        if (!settled)
          finish(new Error("runtime closed the connection before responding"));
      });
      socket.once("connect", () => {
        try {
          sent = true;
          socket.write(
            `${JSON.stringify({ id: requestId, version, method, params })}\n`,
          );
        } catch (error) {
          finish(error);
        }
  ...
  async waitForDaemon(version) {
    ...
    throw new Error(
      `${RUNTIME_UNAVAILABLE_ERROR} ${messageFromError(lastError)}`.trim(),
    );
  }
  ...
  async startDaemon(previousError) {
    ...
    try {
      ...
      throw new Error(
        `${RUNTIME_UNAVAILABLE_ERROR} ${messageFromError(error)}`.trim(),
        { cause: previousError },
      );
    }
  }
