...
function createHarness() {
  ...
  return { commands, handlers, invoke, context, notifications, pi, status };
}
...
test("session lifecycle projects status, ignores agent_end, and cleans up idempotently", async () => {
  const harness = createHarness();
  registerPiSandExtension(harness.pi);
  ...
  // agent_end is intentionally not a registered lifecycle boundary.
