...
import { acquireDatabaseLock, databaseLockPath } from "../src/database-lock.js";
import { readProcessIdentity } from "../src/process.js";
import {
  createAgentServer,
...
test("database ownership rejects the exact live owner but reclaims PID-reused and prior-boot markers", {
  skip: process.platform === "linux" ? false : "Linux-only",
}, async () => {
  const directory = await mkdtemp(join(tmpdir(), "pi-sand-owner-identity-"));
  const dbPath = join(directory, "state.sqlite");
  ...
  try {
    ...
    await writeFile(
      ...
      JSON.stringify({
        pid: identity.pid,
        processStartIdentity: "different-start",
        bootId: identity.bootId,
        token: "reused-pid",
      }),
    );
    const reclaimedPidLock = acquireDatabaseLock(dbPath);
    reclaimedPidLock.release();
    ...
    const reclaimedBootLock = acquireDatabaseLock(dbPath);
    reclaimedBootLock.release();
  } finally {
    await rm(directory, { recursive: true, force: true });
  }
});
...
test("a failed AgentService bootstrap releases ownership so product Retry works in the same server", async () => {
  ...
  try {
    ...
    assert.equal(
      runtime.service,
      undefined,
      "the degraded server must not retain a partially initialized service",
    );
    assert.ok(runtime.startupError);
    assert.equal(
      existsSync(databaseLockPath(dbPath)),
      false,
      "failed initialization must release the database ownership marker",
    );
    ...
    assert.ok(
      runtime.service,
      "Retry must establish the service without restarting the product host",
    );
...
test("normal Desktop launch invokes the located Chromium runtime directly", {
  skip: !chromiumPath,
}, () => {
  const calls = [];
  const child = { unref() {} };
...
test("the product server is loopback-only and rejects arbitrary browser-origin mutations", async () => {
  ...
  try {
    ...
    assert.match(
      ...
      /only available to the pi-sand Desktop/,
    );
...
test("failed service bootstrap still opens the Desktop for product-level Retry", async () => {
  ...
  assert.deepEqual(
    calls,
    ["http://127.0.0.1:4318"],
    "the Desktop must open even when bootstrap cannot establish the service",
  );
});
...
test("cold launch starts a real detached Local Agent Service without manual port selection", async () => {
  ...
  let pid;
  try {
    const launched = await launchProduct({
      port,
      dbPath: join(directory, "state.sqlite"),
      openBrowser: false,
    });
...
test("the supported Chromium Desktop renders the two-pane cold shell", {
  skip: !chromiumPath,
}, async () => {
