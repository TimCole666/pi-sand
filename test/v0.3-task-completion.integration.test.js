...
function deterministicWorker({
  ...
  return async ({ cwd, onEvent }) => {
    ...
    if (failCheckpoint)
      await writeFile(join(cwd, "checkpoint-failure.txt"), "failure\n");
    onEvent({
      type: "message_end",
      message: {
        role: "assistant",
        content: [{ type: "text", text: "A bounded result." }],
        stopReason: "stop",
      },
    });
...
process.stdin.on("data", (chunk) => {
  ...
  while (buffer.includes("\\n")) {
    ...
    if (request.type === "set_model") send({ type: "response", command: request.type, id: request.id, success: true, data: { provider: request.provider, id: request.modelId } }, Number(process.env.PI_SAND_MODEL_DELAY || 0));
...
async function runClient(env, method, params) {
  ...
  return new Promise((resolveClient, rejectClient) => {
    ...
    child.once("close", (code) =>
      code === 0
        ? resolveClient(JSON.parse(stdout))
        : rejectClient(new Error(`client exited ${code}: ${stderr}`)),
    );
  });
}
...
test("RuntimeClient allows the acknowledged startup window to exceed one slow model response", {
  skip: process.platform === "linux" ? false : "Linux-only",
}, async () => {
  const parent = await mkdtemp(join(tmpdir(), "pi-sand-v03-slow-handshake-"));
  const source = await repository(parent);
  const fake = await fakePi(parent);
  const env = {
    ...process.env,
    PI_BIN: fake.command,
    PI_SAND_FAKE_LOG: fake.log,
    PI_SAND_MODEL_DELAY: "6000",
  ...
  try {
    ...
    assert.ok(["running", "completed"].includes(started.task.state));
    assert.equal(started.task.attempts[0].provider, "provider");
  } finally {
...
test("daemon settles the initial AttemptRun after the submitting client disappears without completing the Task", {
  ...
  const parent = await mkdtemp(
    join(tmpdir(), "pi-sand-v03-completion-boundary-"),
  );
  const source = await repository(parent);
  const fake = await fakePi(parent);
  const env = {
...
test("Fresh Executor history records an accepted and settled AttemptRun when prompt acceptance and agent_settled share startup", async () => {
  ...
  const runtime = new RuntimeStore({
    ...startOptions(source, workerFactory, piCommand),
...
test("initial AttemptRun settlement leaves candidate work for Supervisor verification", {
  ...
  for (const scenario of cases) {
    const parent = await mkdtemp(
      join(tmpdir(), `pi-sand-v03-settle-${scenario.name}-`),
    );
    const source = await repository(parent);
    const piCommand = await versionCommand(parent);
    const runtime = new RuntimeStore({
      ...startOptions(source, scenario.worker, piCommand),
      dbPath: join(parent, "runtime.sqlite"),
    });
    try {
...
test("settled nonterminal work keeps the active worker and blocks capacity", {
  ...
  const workerFactory = async (options) => {
    const worker = await deterministicWorker({
      events: [],
      change: true,
      close() {},
    })(options);
